...
export default function PropertyDetail() {
  ...
  // Determine props based on ID
  const isVilla = id && id.includes('villa');
  const name = isVilla ? "The Oasis Concrete Villa" : "Skyline Stone Residences";
  const desc = isVilla 
        ...
        <div style={{ flex: '1 1 450px', display: 'flex', flexDirection: 'column', position: 'sticky', top: '8rem' }}>
          ...
          <div style={{ display: 'flex', gap: '1rem', marginBottom: '2rem' }}>
            <div style={{ flex: 1, padding: '1rem 1.5rem', background: '#FFF', border: '1px solid #EAEAEA', borderRadius: '12px', boxShadow: '0 4px 10px rgba(0,0,0,0.02)' }}>
              ...
              <span style={{ fontSize: '1.1rem', fontWeight: 600, color: 'var(--text-dark)' }}>Prime District</span>
            </div>
            <div style={{ flex: 1, padding: '1rem 1.5rem', background: '#FFF', border: '1px solid #EAEAEA', borderRadius: '12px', boxShadow: '0 4px 10px rgba(0,0,0,0.02)' }}>
              <strong style={{ display: 'block', color: 'var(--text-muted)', fontSize: '0.85rem' }}>Status</strong>
              <span style={{ fontSize: '1.1rem', fontWeight: 600, color: 'var(--primary-dark)' }}>Available</span>
            </div>
          </div>
          <div style={{ borderRadius: '16px', overflow: 'hidden', height: '300px', boxShadow: '0 10px 30px rgba(0,0,0,0.05)' }}>
             <img src={isVilla ? '/listing_villa.png' : '/listing_apt.png'} alt={name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
          </div>
        </div>
